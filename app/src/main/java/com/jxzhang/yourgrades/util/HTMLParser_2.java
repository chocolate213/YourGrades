...
public class HTMLParser_2 {
    ...
    String mData;
    public HTMLParser_2(String data){
        mData = data;
        mPaster();
    }
    ...
    public void mPaster(){
        ...
        Elements classNames = doc.select("td[width=24%]");	//带有width=23%属性的td元素	里面装的是课程名称
       ...
       // for (Element element :classesType){
        ...
        for (Element element : classNames){
            ...
            StudentInfo si = new StudentInfo(className,grade[j]);
            j = j + 5;
            list.add(si);
        }
    }
